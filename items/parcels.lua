...
--highvaluegoods
local highvaluegoods = SMODS.Consumable {
    set = "Parcel",
    name = "draft-highvaluegoods",
    key = "highvaluegoods",
    pos = {x = 9, y = 0},
    atlas = 'parcel_atlas',
    cost = 0,
    order = 1,
    config = {extra = {cost = 0, amount = 0, base_num = 2, num_additional = 4, num_special=10}},
    loc_vars = function(self, info_queue, card)
		local num_special = card.ability.extra.num_special
        local num_normal = - card.ability.extra.num_special + card.ability.extra.num_additional
        for key, value in pairs(SMODS.Ranks) do
            num_normal = num_normal + card.ability.extra.base_num
        end
        return { vars = {G.FUNCS.format_cost(card.ability.extra.cost), num_special, num_normal} }
    end,
    can_use = function(self, card)
        return true
    end,
    use = function(self, card, area, copier)
        ...
        pseudoshuffle(possibilities, pseudoseed('draft_highvaluegoods'))
        for i = 1, card.ability.extra.num_additional, 1 do
            amount_per_rank[possibilities[(i % #possibilities) + 1]] = amount_per_rank[possibilities[(i % #possibilities) + 1]] + 1
        end
        local special_per_rank = {}
        for key, val in pairs(SMODS.Ranks) do
            special_per_rank[val] = 0
        end
        local possibilities = {}
        for key, val in pairs(SMODS.Ranks) do
            table.insert(possibilities, val)
        ...
        pseudoshuffle(possibilities, pseudoseed('draft_highvaluegoods'))
        for i = 1, card.ability.extra.num_special, 1 do
            special_per_rank[possibilities[(i % #possibilities) + 1]] = special_per_rank[possibilities[(i % #possibilities) + 1]] + 1
        end
        for key, value in pairs(SMODS.Ranks) do
            local possibilities = {}
            for key, val in pairs(G.P_CARDS) do
            ...
            pseudoshuffle(possibilities, pseudoseed('draft_highvaluegoods'))
            for i = 1, amount_per_rank[value], 1 do
                if special_per_rank[value] >= i then
                    create_playing_card({
                        front = possibilities[i],
